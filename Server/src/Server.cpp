...
{
	std::u16string filePath = u"Monitors.kaa";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;

	std::u16string header = reader.readU16String();
	if (header != u"Kulikova")
	{
		reader.close();
		return;
	}
...
void Server::saveMonitors()
{
	...
	writer.writeU16String(u"Kulikova");
...
{
	std::u16string filePath = u"Scanners.kaa";
	BinaryReader reader(filePath);
	if (!reader.isOpen())
		return;
...
void Server::startUp()
{
	base = std::shared_ptr<Base>(BaseLoader::load(u"Base.kaa"));
	threats = std::make_shared<ThreatList>(u"Threats.kaa");
	threats->load();
